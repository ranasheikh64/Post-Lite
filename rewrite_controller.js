...
const path = 'lib/app/modules/request_builder/request_builder_controller.dart';
let content = fs.readFileSync(path, 'utf8');

// Replace class definition up to onInit
const newTop = `import 'package:dio/dio.dart';
...
content = content.replace(/import.*?@override/s, newTop);

// Replace onInit
const newOnInit = `void onInit() {
    super.onInit();
...
content = content.replace(/void onInit\(\) \{.*?\}/s, newOnInit);

// Add sync methods and replace _checkUnsavedChanges
const newSyncAndCheck = `void syncUrlToParams() {
    if (_isParsingUrl) return;
    _isParsingUrl = true;
...
content = content.replace(/void _checkUnsavedChanges\(\) \{.*?\}/s, newSyncAndCheck);

// Replace loadRequest
const newLoadRequest = `void loadRequest(Map<String, dynamic> request, {String path = 'Workspace > Collection'}) {
...
content = content.replace(/void loadRequest\(.*?responseSize\.value = 0;\n  \}/s, newLoadRequest);

// Replace saveChanges
const newSaveChanges = `Future<void> saveChanges() async {
...
content = content.replace(/Future<void> saveChanges\(\) async \{.*?Get\.snackbar\('Error', 'Failed to save request'\);\n    \}\n  \}/s, newSaveChanges);

// Update sendRequest to use headers and body correctly
const newSendRequest = `void sendRequest() async {
...
content = content.replace(/void sendRequest\(\) async \{.*?isLoading\.value = false;\n    \}\n  \}/s, newSendRequest);
